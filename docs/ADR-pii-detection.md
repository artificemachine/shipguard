...
- **`.csv` / `.log`** — see §3.
- **Generic engine-level `max-findings-per-file`** — the *proper* fix for the flood risk. Belongs in `engine.py` where findings from all rules aggregate, configurable via `.shipguard.yml`, applying to every rule category. `pii.py` ships a local `_cap_findings` instead because the engine-level version is a scope expansion no other rule family currently needs. **This is a known gap, deliberately taken.** When the engine-level cap lands, delete `_cap_findings` from `pii.py`.
- **AI-triage** — see §5.
